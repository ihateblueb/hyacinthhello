...
object Metrics {
    ...
    fun register() {
        ...
        metrics.addCustomChart(SimplePie("character_limit") {
            HyacinthHello.instance.config.getString(
                "maximum-message-length",
                "60"
            )
        })
        metrics.addCustomChart(SimplePie("proxy_mode") {
            HyacinthHello.instance.config.getString(
                "proxy-mode",
                "false"
            )
        ...
        metrics.addCustomChart(SimplePie("economy_enabled") {
            HyacinthHello.instance.config.getString(
                "economy.enabled",
                "false"
            )
